package com.tony.electrovalencedemo2.service;


import com.tony.electrovalencedemo2.util.TokenUtil;
...
//        SortedSetOperate();
//        HashOperate();
        String strToken = this.GetToken(userName);
    ...
    public String GetToken(String userName) {
        ...
        if (jedis != null) {
            ...
            strToken = shardedJedis.hget(userName, "token");
            String strTokenTime = shardedJedis.hget(userName, "tokenTimestamp");


            if (strToken == null || strToken == "") {
                strResultToken = GetTokenByUrl(userName);
                //strResultToken="test1";

                shardedJedis.hset(userName, "token", strResultToken);
                shardedJedis.hset(userName, "tokenTimestamp", System.currentTimeMillis() + "");

            } else {
                if (strTokenTime != null && strTokenTime != null) {
                    long tokenS = Long.parseLong(strTokenTime);
...
//                    if (result2 > 1) {
                    if (result >= 23) {
                        strResultToken = GetTokenByUrl(userName);
                        System.out.println(strResultToken);

                        shardedJedis.hdel(userName, "tokenTimestamp");
